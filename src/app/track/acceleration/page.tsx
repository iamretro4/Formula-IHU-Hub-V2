...
export default function AccelerationEventPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // Load acceleration runs for today
  ...
  const onSubmit = async (data: RunFormData) => {
    ...
    try {
      ...
      const totalPenalty = conePenalty + offCoursePenalty
      const correctedTime = data.raw_time + totalPenalty

      const runData = {
        team_id: data.team_id,
        driver_id: data.driver_id || null,
        event_type: 'acceleration' as const,
        ...
        notes: data.notes,
        weather_conditions: 'Clear' // Could be dynamic
      }
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center p-8">
        ...
        <span className="ml-2">Loading acceleration event...</span>
      </div>
    )
  }
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Acceleration Event</h1>
            <p className="text-gray-600">75m straight line acceleration test</p>
          </div>
        </div>
            ...
            <CardContent>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  {form.formState.errors.run_number && (
                    <p className="text-sm text-red-600">
                      {form.formState.errors.run_number.message}
                    </p>
                  )}
                </div>
                        ...
                        <TableCell>
                          {run.penalties && typeof run.penalties === 'object' ? (
                            <div className="text-xs">
                              ...
                              {(run.penalties as any).cones === 0 && (run.penalties as any).off_course === 0 && (
                                <div>None</div>
                              )}
                            </div>
                          ) : (
